...
def format_stats_df(stats, group_col):
    ...
    result_df["1차 환자성명 확인"] = (
        ...
        + "건 ("
        + stats["1차확인_비율"].apply(format_pct)
        + "%)"
    )
    result_df["2차 등록번호 확인"] = (
        stats["차2확인_성공"].astype(str)
        + "건 ("
        + stats["2차확인_비율"].apply(format_pct)
        + "%)"
    )
    result_df["정확한 환자확인"] = (
        stats["정확한확인_성공"].astype(str)
        + "건 ("
        + stats["정확한확인_비율"].apply(format_pct)
        + "%)"
    )
    return result_df
...
if uploaded_file is not None:
    try:
        ...
        final_rate_str = format_pct(final_rate_val)
        ...
        fig_job.add_trace(
            go.Bar(
                ...
                text=[
                    f"1차: {format_pct(val)}%"
                    for val in raw_job_filtered["1차확인_비율"]
                ],
                textposition="inside",
        ...
        fig_job.add_trace(
            go.Bar(
                ...
                text=[
                    f"2차: {format_pct(val)}%"
                    for val in raw_job_filtered["2차확인_비율"]
                ],
                textposition="inside",
                insidetextfont=dict(size=20, color="white"),
                marker_color="#109618",
        ...
        for idx, row in raw_job_filtered.iterrows():
            ...
            fig_job.add_annotation(
                x=row["직군"],
                y=total_height + 8,
                text=f"<b>정확한 확인율: {format_pct(row['정확한확인_비율'])}%</b>",
                showarrow=False,
        ...
        res_dept_df["1차 환자성명 확인"] = (
            ...
            + "건 ("
            + raw_dept_sub["1차확인_비율"].apply(format_pct)
            + "%)"
        )
        res_dept_df["2차 등록번호 확인"] = (
            raw_dept_sub["차2확인_성공"].astype(str)
            + "건 ("
            + raw_dept_sub["2차확인_비율"].apply(format_pct)
            + "%)"
        )
        res_dept_df["정확한 환자확인"] = (
            raw_dept_sub["정확한확인_성공"].astype(str)
            + "건 ("
            + raw_dept_sub["정확한확인_비율"].apply(format_pct)
            + "%)"
        )
